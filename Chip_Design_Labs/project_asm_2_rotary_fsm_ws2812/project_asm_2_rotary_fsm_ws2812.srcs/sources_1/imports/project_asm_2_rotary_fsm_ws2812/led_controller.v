module led_controller #(
    ...
    localparam DELAY_SUPER = 2_000_000;         // 16ms @125MHz
    localparam DELAY_FAST  = 20_000_000;        // 160ms @125MHz
    ...
    // ----------------------------------------------------------
    // WS2812 CHAIN (giữ nguyên module - module đã hoàn thiện)
    // ----------------------------------------------------------
    ws2812_chain #(
        .NUM_LEDS(NUM_LEDS)
    ) chain_inst (
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // delay để làm mượt fade (125 MHz -> ~16ms mỗi step)
            if (delay_cnt < refresh_max)
                ...
                blink_state <= ~blink_state; // đảo color
    ...
    function [23:0] get_color;
        ...
        begin
            case (color_id)
                2'd0: get_color = {8'h00, 8'hFF, 8'h00};           // RED
                2'd1: get_color = {8'hFF, 8'h00, 8'h00};           // GREEN
                2'd2: get_color = {8'h00, 8'h00, 8'hFF};           // BLUE
                2'd3: get_color = {phase, phase >> 1, ~phase};     // RAINBOW-ish
    ...
    always @(*) begin
        ...
        for (i = 0; i < NUM_LEDS; i = i + 1) begin
            ...
            case (mode)
                ...
                2'd0: begin
                    led_data[i*24 +: 24] = (i == idx) ? col : 24'h000000;
                end
...
//                    led_data[i*24 +: 24] = ((i & 1) == idx[0]) ? col : 24'h000000;
                    if (blink_state == 0)
                        led_data[i*24 +: 24] = (i % 2 == 0) ? 24'hFF0000 : 24'h000000; // chẵn sáng green
                    else
                        led_data[i*24 +: 24] = (i % 2 == 1) ? 24'h0000FF : 24'h000000; // lẻ sáng blue
                end
