module top #(
    ...
    input  wire rst,
    input  wire A,          // pha A (CLK)
    input  wire B,          // pha B (DT)
    ...
    rotary_decoder u_rotary (.clk(clk), .rst(rst),
        .A(A), .B(B), .pulse_up(pulse_up), .pulse_down(pulse_down)
    );

    wire btn_short, btn_hold;

    button_manager u_btn (.clk(clk), .rst(rst),
        .btn_raw(~btn),       // <<< FIX ACTIVE-LOW
        .btn_short(btn_short), .btn_hold(btn_hold)
    );
    ...
    // ----------------------------------------------------------
    reg [1:0] menu_state;   // select mode(pattern)/color/speed
    reg [1:0] mode;         // pattern (chase - blink - alternating - fade)
    reg [1:0] color;        // color hex: green - red - blue
    reg [1:0] speed;        // speed delay
